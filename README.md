## Nitin K. Mishra
Developer, 20. Building real systems in production, getting sharper across the stack every day.
I work on the layer between a model and something people can actually use retrieval that returns the right chunk, agents that know when to stop, models tuned for the job.

<br>

### Logorhythms

Building [Logorhythms](https://logorhythms.studio) turning hard AI problems into products that ship.
- **logorhythms.studio** — the studio, in progress
- **[Docuverse](https://logorhythms.in)** — launched

<br>

### Elsewhere

[nitin.vision](https://nitin.vision) &nbsp;·&nbsp; [LinkedIn](https://www.linkedin.com/in/nitin-k-mishra-520615331) &nbsp;·&nbsp; [Email](mailto:nitiniszod10@gmail.com) &nbsp;·&nbsp; [X](https://x.com/Nitin101_)

<br>

### What I build

**RAG & Agents** — hybrid retrieval, multi-agent orchestration, guardrails that fail loudly.

**MCP Servers** — custom tools and connectors that give models real hands.

**LLM Fine-Tuning** — LoRA / PEFT, domain adaptation, evals over vibes.

**Computer Vision** — detection, OCR pipelines, vision + language systems.

**Voice AI** — streaming STT → LLM → TTS, sub-second turn-taking.

**Full-Stack** — the API, the pipeline, the interface, shipped end to end.

<br>

---

*Products before pitch decks.*
